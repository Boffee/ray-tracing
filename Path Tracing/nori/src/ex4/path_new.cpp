...
public:
        ...
        Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &_ray) const {
            // use inLi(...) directly
            return inLi(scene, sampler, _ray, 0.7f, 1);
        }
...
//                }
                ...
                Color3f g = scene->evalTransmittance(Ray3f(lRec.ref, lRec.d, 0, lRec.dist), sampler);
                Color3f c = bsdf->eval(bRec) * s * g * std::abs(Frame::cosTheta(bRec.wo));
